...
internal static partial class RestContractStreamProjector
{
        ...
        where T : ITemplate, IDamlRecord<T>
    {
        if (evt?.CreatedEvent is { } created)
        {
            ...
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<T>.MatchesCreated(created),
                synchronizerId,
        ...
        if (evt?.ArchivedEvent is { } archived)
        {
            ...
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<T>.MatchesArchived(archived),
                synchronizerId,
        ...
        if (evt?.ExercisedEvent is { } exercised)
        {
            ...
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<T>.MatchesExercised(exercised),
                synchronizerId,
        ...
        where T : ITemplate, IDamlRecord<T>
    {
        ...
        if (evt?.JsUnassignedEvent is { } unassigned)
        {
            ...
            var decoded = new DecodedStreamEvent<ReassignmentScope>(
                offset,
                RestMarkerMatcher<T>.MatchesUnassigned(unassigned),
    ...
    private static bool TryResolveCreatedPayload<T>(WireCreatedEvent created, out T payload)
        where T : ITemplate, IDamlRecord<T>
    {
        if (RestMarkerMatcher<T>.IsInterface)
        {
            if (!RestMarkerMatcher<T>.TryGetInterfaceViewRecord<T>(created, out var view))
            {
                payload = default!;
                return false;
            }
        ...
        payload = T.FromRecord(RestValueDecoder.ToDamlRecord<T>(RestPayloadDecoder.RequireCreateArgument(created)));
        return true;
    }
