...
internal static class RestInterfaceStreamProjector
{
        ...
        where TView : IDamlRecord<TView>
    {
        if (!RestWireConversions.TryParseOffset(transaction.Offset, out var transactionOffset))
        {
            yield return Refused<TInterface, TView>(
                RestContractStreamProjector.UnparseableOffsetRefusal(typeof(TInterface).Name, transaction.Offset, logger));
            yield break;
        }
        ...
        where TView : IDamlRecord<TView>
    {
        if (evt?.CreatedEvent is { } created)
        {
            ...
            RestContractStreamProjector.RequireTemplateId(created.TemplateId, nameof(Raw.CreatedEvent), created.ContractId);
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<TInterface>.MatchesCreated(created),
                synchronizerId,
        ...
        if (evt?.ArchivedEvent is { } archived)
        {
            ...
            RestContractStreamProjector.RequireTemplateId(archived.TemplateId, nameof(Raw.ArchivedEvent), archived.ContractId);
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<TInterface>.MatchesArchived(archived),
                synchronizerId,
        ...
        if (evt?.ExercisedEvent is { } exercised)
        {
            ...
            RestContractStreamProjector.RequireTemplateId(exercised.TemplateId, nameof(Raw.ExercisedEvent), exercised.ContractId);
            var decoded = new DecodedStreamEvent<SynchronizerId>(
                offset,
                RestMarkerMatcher<TInterface>.MatchesExercised(exercised),
                synchronizerId,
        ...
        where TView : IDamlRecord<TView>
    {
        ...
        {
            yield return Refused<TInterface, TView>(
                RestContractStreamProjector.UnparseableOffsetRefusal(typeof(TInterface).Name, reassignment.Offset, logger));
            yield break;
        }
        ...
        where TView : IDamlRecord<TView>
    {
        if (evt?.JsAssignmentEvent is { } assigned)
        {
            ...
            return new InterfaceStreamEvent<TInterface, TView>.Assigned(
                ...
                view,
                RestContractStreamProjector.ContractKeyOf(created),
        ...
        if (evt?.JsUnassignedEvent is { } unassigned)
        {
            ...
            RestContractStreamProjector.RequireTemplateId(unassigned.TemplateId, nameof(Raw.UnassignedEvent), unassigned.ContractId);
            var decoded = new DecodedStreamEvent<ReassignmentScope>(
                offset,
                RestMarkerMatcher<TInterface>.MatchesUnassigned(unassigned),
        ...
        where TView : IDamlRecord<TView>
    {
        ...
        if (!RestContractStreamProjector.TryParseOptionalOffset(unassignmentWireOffset, snapshotOffset, out var unassignmentOffset))
        {
            yield return Refused<TInterface, TView>(RestContractStreamProjector.UnparseableOffsetInSnapshotRefusal(
        ...
        where TView : IDamlRecord<TView>
    {
        ...
        {
            return Refused<TInterface, TView>(RestContractStreamProjector.UnparseableOffsetInSnapshotRefusal(
    ...
    private static bool TryResolveView<TInterface, TView>(WireCreatedEvent created, out TView view)
        ...
        where TView : IDamlRecord<TView>
    {
        if (!RestMarkerMatcher<TInterface>.TryGetInterfaceViewRecord<TView>(created, out var record))
        {
            view = default!;
            return false;
        }
