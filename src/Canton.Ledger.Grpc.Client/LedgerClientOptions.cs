...
public class LedgerClientOptions : IValidatableObject
{
    ...
    /// which is what a Canton participant demanding mutual TLS requires. Configured TLS requires an
    /// <c>https</c> <see cref="GrpcAddress"/>; validation rejects material that an <c>http</c>
    /// channel would silently ignore.
    /// </summary>
    /// <remarks>
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (Tls.IsConfigured
            && Uri.TryCreate(GrpcAddress, UriKind.Absolute, out var endpoint)
            && endpoint.Scheme == Uri.UriSchemeHttp)
        {
            nestedResults.Add(new ValidationResult(
                $"{nameof(Tls)} is configured, but {nameof(GrpcAddress)} uses plaintext http. "
                + "TLS identity and trust settings require https.",
                [nameof(Tls), nameof(GrpcAddress)]));
        }
