...
/// move on the target. Submitted through <see cref="ICantonLedgerClient.SubmitReassignmentAsync"/>
/// (fire) or <see cref="ICantonLedgerClient.TrySubmitAndWaitForReassignmentAsync{T}"/> (await),
/// per ADR 0007.
/// </summary>
...
/// synchronizer ids are required — a reassignment names both endpoints explicitly (ADR 0004),
...
/// reassignment id from the unassign result and passes it here (ADR 0007). The source and target
...
/// write path (ADR 0007). Construct with <see cref="Of"/> and refine with the <c>With…</c> members.
/// </summary>
public sealed record ReassignmentSubmission
{
    ...
    /// reported back from the submit call (ADR 0007).
    /// </summary>
