...
/// </description></item>
/// <item><description>
/// <see cref="ClientCertificate"/>, when supplied, carries a private key.
/// </description></item>
/// <item><description>
...
/// </description></item>
/// <item><description>
/// <see cref="RevocationMode"/> is a defined <see cref="X509RevocationMode"/> value.
/// </description></item>
/// <item><description>
...
/// does not resolve surfaces as a <see cref="System.IO.FileNotFoundException"/> during transport
/// construction. An unresolvable
...
{
    /// <summary>
    /// Path to the PEM file holding the client certificate and any intermediate certificates.
    /// When <see cref="ClientCertificateKeyPemPath"/> is <see langword="null"/>, empty or
    /// whitespace this same file is also read for the private key, which covers the combined
    ...
    /// <summary>
    /// Path to the PKCS#12 (<c>.pfx</c> / <c>.p12</c>) file holding the client certificate, its
    /// private key and any intermediate certificates.
    /// </summary>
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        if (ClientCertificate is { HasPrivateKey: false })
            yield return new ValidationResult(
                $"TlsOptions.{nameof(ClientCertificate)} must carry a private key.",
                [nameof(ClientCertificate)]);
        ...
        if (!Enum.IsDefined(RevocationMode))
            yield return new ValidationResult(
                $"TlsOptions.{nameof(RevocationMode)} has an undefined value: {RevocationMode}.",
                [nameof(RevocationMode)]);
        else if (RevocationMode != X509RevocationMode.NoCheck
            && !HasCertificateAuthorityBundle
