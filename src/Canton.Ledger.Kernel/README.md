...
BCL `System.Diagnostics.Activity` only — no OpenTelemetry SDK dependency (ADR 0010). `StartActivity` no-ops (returns `null`) until the host registers a listener.
