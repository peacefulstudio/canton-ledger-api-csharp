...
public class ActAsRightsLeaseTests
{
    ...
    [Fact]
    public async Task DisposeAsync_sends_nothing_when_the_grant_was_definitively_rejected_with_4xx()
    {
        using var participant = new RecordingParticipant(
    ...
    [Fact]
    public async Task DisposeAsync_revokes_a_grant_after_its_response_is_lost()
    {
        var revoke = RevokingOnly(Alice);
        using var participant = new RecordingParticipant(
            respondTo: request => request.Method == HttpMethod.Post
        ...
        var revokeRequest = participant.Requests.Should().HaveCount(2).And.Subject.Last();
        revokeRequest.Method.Should().Be(HttpMethod.Patch);
        ActAsPartiesIn(revokeRequest.Body).Should().Equal(Alice);
    }

    [Fact]
    public async Task DisposeAsync_revokes_a_grant_after_a_client_timeout()
    {
        var revoke = RevokingOnly(Alice);
        using var participant = new RecordingParticipant(
            respondTo: request => request.Method == HttpMethod.Post
    ...
    [Fact]
    public async Task DisposeAsync_revokes_a_grant_after_an_ambiguous_5xx_response()
    {
        var revoke = RevokingOnly(Alice);
        using var participant = new RecordingParticipant(
            respondTo: request => request.Method == HttpMethod.Post
                ...
                {
                    Content = new StringContent("gateway lost the participant response")
                }
                : revoke(request));
        var lease = NewLease(participant);
    ...
    [Fact]
    public async Task DisposeAsync_revokes_a_grant_after_an_ambiguous_408_response()
    {
        var revoke = RevokingOnly(Alice);
        using var participant = new RecordingParticipant(
            respondTo: request => request.Method == HttpMethod.Post
                ...
                {
                    Content = new StringContent("participant outcome unavailable")
                }
                : revoke(request));
        var lease = NewLease(participant);
    ...
    [Fact]
    public async Task DisposeAsync_accepts_an_ambiguous_grant_that_was_already_absent()
    {
        var revoke = RevokingOnly();
        using var participant = new RecordingParticipant(
            respondTo: request => request.Method == HttpMethod.Post
                ? new HttpResponseMessage(HttpStatusCode.BadGateway)
                {
                    Content = new StringContent("upstream response unavailable")
                }
                : revoke(request));
        var lease = NewLease(participant);
        ...
        participant.Requests.Should().HaveCount(2);
        participant.Requests.Last().Method.Should().Be(HttpMethod.Patch);
    }

    [Fact]
    public async Task DisposeAsync_sends_nothing_when_access_token_acquisition_fails()
    {
        using var participant = new RecordingParticipant();
        var lease = new ActAsRightsLease(
            new Uri(JsonLedgerApi),
            UserId,
            _ => throw new InvalidOperationException("token unavailable"),
            participant);
