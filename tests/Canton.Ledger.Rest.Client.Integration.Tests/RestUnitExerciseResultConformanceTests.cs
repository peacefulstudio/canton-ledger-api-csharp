...
/// <summary>
/// LocalNet conformance coverage that a unit-returning choice decodes to <see cref="DamlUnit"/>
/// against its generated choice descriptor. The consuming <c>Archive</c> choice on the shared
/// <c>richtypes.dar</c> <c>Marker</c> returns unit, and
...
/// separates unit from a record with no fields or an empty <c>TextMap</c>, so only the Daml type the
/// loaded generated code declares for the choice result can tell them apart.
/// </para>
/// <para>
/// The decoded leg feeds those same served bytes through the deserializer the client reads replies
...
public class RestUnitExerciseResultConformanceTests
{
    ...
    [Fact]
    public async Task A_unit_returning_choice_is_served_as_a_bare_empty_object_and_decodes_to_DamlUnit_against_its_choice_descriptor()
    {
        ...
        exerciseResult.EnumerateObject().Should().BeEmpty(
            ...
            + "empty TextMap, so only the generated choice descriptor can resolve it, and it "
        ...
        RestTransactionResultProjector.Project(reply.Transaction)
            ...
            .Which.ExerciseResult.Should().Be(
                DamlUnit.Instance,
                "the generated Marker.ChoiceArchive descriptor declares a unit result, which is the "
                + "only thing that resolves the served bare empty object");
    }
