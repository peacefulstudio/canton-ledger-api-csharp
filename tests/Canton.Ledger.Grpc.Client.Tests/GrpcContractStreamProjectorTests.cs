...
public class GrpcContractStreamProjectorTests
{
    ...
    public void ProjectTransactionEvents_surfaces_Created_with_undecodable_create_arguments_as_decode_failure_at_the_transactions_offset(ProtoValue poisonValue)
    {
        ...
        var events = GrpcContractStreamProjector.ProjectTransactionEvents<TemplateMarker>(transaction).ToList();
    ...
    public void ProjectReassignmentEvents_surfaces_Assigned_with_undecodable_payload_as_decode_failure_at_the_reassignments_offset(ProtoValue poisonValue)
    {
        ...
        var events = GrpcContractStreamProjector.ProjectReassignmentEvents<TemplateMarker>(reassignment).ToList();
    ...
    [Fact]
    public void ProjectTransactionEvents_logs_a_MalformedResponseException_for_an_undecodable_contract_key_and_still_yields_a_decode_failure()
    {
        var created = new ProtoCreatedEvent
        {
            ContractId = "00badkey",
            TemplateId = new ProtoIdentifier { PackageId = "tmpl-pkg", ModuleName = "Sample.Token", EntityName = "Holding" },
            CreateArguments = LedgerClientTestFixtures.OwnerArguments(),
            ContractKey = UnsetSumValue(),
            Offset = 60L,
        };
        var transaction = new Transaction { Offset = 59L, SynchronizerId = "sync-1" };
        transaction.Events.Add(new Event { Created = created });
        ...
        var unclassified = events.Should().ContainSingle().Subject
            .Should().BeOfType<ContractStreamEvent<TemplateMarker>.Unclassified>().Subject;
        unclassified.Offset.Should().Be(LedgerOffset.At(59L));
        unclassified.Kind.Should().Be(UnclassifiedKind.DecodeFailure);
    ...
    public void ProjectTransactionEvents_leaves_Created_Key_null_when_the_wire_carries_no_key()
    {
        ...
        GrpcContractStreamProjector.ProjectTransactionEvents<TemplateMarker>(transaction)
            .Should().ContainSingle().Subject
            .Should().BeOfType<ContractStreamEvent<TemplateMarker>.Created>().Subject
            .Key.Should().BeNull();
    }
    ...
    [Fact]
    public void ProjectTransactionEvents_surfaces_a_matching_exercise_without_a_choice_argument_as_DecodeFailure()
    {
        var exercised = MatchingExerciseWithUnitPayloads();
        exercised.ChoiceArgument = null;

        AssertTemplateStreamDecodeFailure(
            exercised,
    ...
    [Fact]
    public void ProjectTransactionEvents_surfaces_a_matching_exercise_without_an_exercise_result_as_DecodeFailure()
    {
        var exercised = MatchingExerciseWithUnitPayloads();
    ...
    [Fact]
    public void ProjectTransactionEvents_surfaces_a_matching_created_event_without_create_arguments_as_DecodeFailure()
    {
        var transaction = new Transaction { Offset = 150L, SynchronizerId = "sync-1" };
        transaction.Events.Add(new Event
        {
            Created = new ProtoCreatedEvent
            {
        ...
        events.Should().ContainSingle().Which.Should().BeOfType<ContractStreamEvent<TemplateMarker>.Unclassified>()
            .Which.Kind.Should().Be(UnclassifiedKind.DecodeFailure);
        loggerFactory.Records.Should().ContainSingle(record => record.Level == LogLevel.Warning)
            .Which.Exception.Should().BeOfType<MalformedResponseException>()
            .Which.Message.Should().Be(
                "Malformed response from ledger: CreatedEvent for contract '00holding' has no create_arguments, though the Ledger API marks the field as required.");
    }

    [Fact]
    public void ProjectTransactionEvents_surfaces_a_matching_interface_exercise_without_a_choice_argument_as_DecodeFailure()
    {
        var exercised = MatchingExerciseWithUnitPayloads();
        exercised.ImplementedInterfaces.Add(MatchingInterfaceId);
        exercised.ChoiceArgument = null;

        AssertInterfaceStreamDecodeFailure(
            exercised,
            "Malformed response from ledger: ExercisedEvent for contract '00holding' has no choice_argument, though the Ledger API marks the field as required.");
    }

    [Fact]
    public void ProjectTransactionEvents_surfaces_a_matching_interface_exercise_without_an_exercise_result_as_DecodeFailure()
    {
        var exercised = MatchingExerciseWithUnitPayloads();
        exercised.ImplementedInterfaces.Add(MatchingInterfaceId);
        exercised.ExerciseResult = null;
    ...
    private static ProtoExercisedEvent MatchingExerciseWithUnitPayloads() => new()
    {
        ContractId = "00holding",
        TemplateId = new ProtoIdentifier { PackageId = "tmpl-pkg", ModuleName = "Sample.Token", EntityName = "Holding" },
        Choice = "Transfer",
    ...
    private static void AssertTemplateStreamDecodeFailure(ProtoExercisedEvent exercised, string expectedMessage)
    {
    ...
    private static void AssertInterfaceStreamDecodeFailure(ProtoExercisedEvent exercised, string expectedMessage)
    {
        ...
        var events = GrpcInterfaceStreamProjector
            .ProjectTransactionEvents<InterfaceMarker, InterfaceMarkerView>(transaction, loggerFactory.CreateLogger("test"))
            .ToList();

        events.Should().ContainSingle().Which.Should().BeOfType<InterfaceStreamEvent<InterfaceMarker, InterfaceMarkerView>.Unclassified>()
            .Which.Kind.Should().Be(UnclassifiedKind.DecodeFailure);
        loggerFactory.Records.Should().ContainSingle(record => record.Level == LogLevel.Warning)
            .Which.Exception.Should().BeOfType<MalformedResponseException>()
            .Which.Message.Should().Be(expectedMessage);
