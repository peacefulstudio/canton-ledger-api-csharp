...
public class LedgerClientOptionsTests
{
    ...
    [Fact]
    public void Validate_rejects_configured_Tls_with_plaintext_GrpcAddress()
    {
        var options = new LedgerClientOptions
        {
        ...
        results.Should().ContainSingle()
            .Which.MemberNames.Should().BeEquivalentTo(
                nameof(LedgerClientOptions.Tls),
                nameof(LedgerClientOptions.GrpcAddress));
    }

    [Fact]
    public void Validate_accepts_unconfigured_Tls_with_plaintext_GrpcAddress()
    {
        var options = new LedgerClientOptions { GrpcAddress = "http://localhost:5001" };
