...
public class DamlValueConverterTests
{
    ...
    [Fact]
    public void from_daml_value_converts_unit_to_object()
    {
        // object is assignable from DamlUnit, so the instance is returned (boxed).
        var result = DamlValueConverter.FromDamlValue<object>(DamlUnit.Instance);

        result.Should().BeSameAs(DamlUnit.Instance);
    }

    [Fact]
    public void from_daml_value_throws_for_unit_to_value_type()
    {
        var action = () => DamlValueConverter.FromDamlValue<int>(DamlUnit.Instance);

        action.Should().Throw<NotSupportedException>()
            .WithMessage("*DamlUnit*value type*Int32*");
    }

    [Fact]
    public void from_daml_value_returns_unit_instance_when_requested_as_daml_unit()
    {
        // Regression: the assignable-type check must run before the DamlUnit special case
        // so that FromDamlValue<DamlUnit>(DamlUnit.Instance) returns the instance, not null.
        var result = DamlValueConverter.FromDamlValue<DamlUnit>(DamlUnit.Instance);

        result.Should().BeSameAs(DamlUnit.Instance);
    }

    [Fact]
    public void from_daml_value_returns_unit_instance_when_requested_as_daml_value()
    {
        var result = DamlValueConverter.FromDamlValue<DamlValue>(DamlUnit.Instance);

        result.Should().BeSameAs(DamlUnit.Instance);
    }

    [Fact]
    public void from_daml_value_returns_null_for_unit_to_nullable_value_type()
    {
        // Nullable<T> is a struct but can represent null — default(int?) is null, a valid "no value".
        var result = DamlValueConverter.FromDamlValue<int?>(DamlUnit.Instance);

        result.Should().BeNull();
    }

    [Fact]
    public void from_daml_value_converts_text_to_string()
    {
        var result = DamlValueConverter.FromDamlValue<string>(new DamlText("hello"));

        result.Should().Be("hello");
    }

    [Fact]
    public void from_daml_value_converts_party_to_string()
    {
        var result = DamlValueConverter.FromDamlValue<string>(new DamlParty("party::alice"));

        result.Should().Be("party::alice");
    }

    [Fact]
    public void from_daml_value_converts_contract_id_to_string()
    {
        var result = DamlValueConverter.FromDamlValue<string>(new DamlContractId("00abc123"));

        result.Should().Be("00abc123");
    }

    [Fact]
    public void from_daml_value_converts_party_to_party_value_type()
    {
        var result = DamlValueConverter.FromDamlValue<Party>(new DamlParty("party::alice"));

        result.Id.Should().Be("party::alice");
    }

    [Fact]
    public void from_daml_value_throws_for_unsupported_value_to_string()
    {
        var action = () => DamlValueConverter.FromDamlValue<string>(new DamlInt64(42));

        action.Should().Throw<NotSupportedException>()
            .WithMessage("*DamlInt64*string*DamlText, DamlParty, and DamlContractId*");
    }

    [Fact]
    public void from_daml_value_converts_int64_to_long()
    {
        var result = DamlValueConverter.FromDamlValue<long>(new DamlInt64(42));

        result.Should().Be(42);
    }

    [Fact]
    public void from_daml_value_converts_bool()
    {
        DamlValueConverter.FromDamlValue<bool>(new DamlBool(true)).Should().BeTrue();
    }

    [Fact]
    public void from_daml_value_converts_numeric_to_decimal()
    {
        var result = DamlValueConverter.FromDamlValue<decimal>(new DamlNumeric(3.14m));

        result.Should().Be(3.14m);
    }

    [Fact]
    public void from_daml_value_converts_date_to_date_only()
    {
        var date = new DateOnly(2024, 6, 15);
        var result = DamlValueConverter.FromDamlValue<DateOnly>(new DamlDate(date));

        result.Should().Be(date);
    }

    [Fact]
    public void from_daml_value_converts_timestamp_to_datetime_offset()
    {
        var ts = DateTimeOffset.UnixEpoch.AddSeconds(1704067200);
        var result = DamlValueConverter.FromDamlValue<DateTimeOffset>(new DamlTimestamp(ts));

        result.Should().Be(ts);
    }

    [Fact]
    public void from_daml_value_returns_daml_value_directly()
    {
        var text = new DamlText("hello");

        var result = DamlValueConverter.FromDamlValue<DamlText>(text);

        result.Should().BeSameAs(text);
    }

    [Fact]
    public void from_daml_value_returns_base_daml_value()
    {
        var text = new DamlText("hello");

        var result = DamlValueConverter.FromDamlValue<DamlValue>(text);

        result.Should().BeSameAs(text);
    }

    [Fact]
    public void from_daml_value_returns_daml_record()
    {
        var record = new DamlRecord(null, [new DamlField("x", new DamlInt64(1))]);

        var result = DamlValueConverter.FromDamlValue<DamlRecord>(record);

        result.Should().BeSameAs(record);
    }

    [Fact]
    public void from_daml_value_converts_contract_id()
    {
        var cid = new DamlContractId("00abc123");

        var result = DamlValueConverter.FromDamlValue<ContractId<TestTemplate>>(cid);

        result.Value.Should().Be("00abc123");
    }

    [Fact]
    public void from_daml_value_throws_for_unsupported_type()
    {
        var action = () => DamlValueConverter.FromDamlValue<DateTime>(new DamlText("nope"));
