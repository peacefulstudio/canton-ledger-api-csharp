...
public class LedgerBuilderTests
{
    ...
    [Fact]
    public void LedgerOutcomes_CommittedUndecodable_carries_the_update_id_message_and_source_exception()
    {
        var source = new FormatException("bad offset");

        var outcome = LedgerOutcomes.CommittedUndecodable<string>("update-1", "could not decode", source);
        ...
        undecodable.Message.Should().Be("could not decode");
        undecodable.SourceException.Should().BeSameAs(source);
    }
