...
public sealed class SslClientAuthenticationOptionsFactoryTests : IDisposable
{
    ...
    [InlineData("")]
    [InlineData("   ")]
    public async Task Create_loads_a_combined_PEM_when_ClientCertificateKeyPemPath_is_blank(string keyPath)
    {
        var presented = await HandshakeAsync(new TlsOptions
        {
            ClientCertificatePemPath = _material.WriteCombinedPem("client-combined-blank-key.pem", _clientCertificate),
            ClientCertificateKeyPemPath = keyPath,
            CertificateAuthorities = [_certificateAuthority]
        });
    ...
    [Fact]
    public void Create_preserves_intermediate_certificates_from_a_PEM_identity_bundle()
    {
        var intermediate = _material.IssueCertificateAuthority(_certificateAuthority, "Canton Intermediate Root");
        var client = _material.IssueClientCertificate(intermediate, "Canton Intermediate Client");

        var authenticationOptions = SslClientAuthenticationOptionsFactory.Create(new TlsOptions
        {
            ClientCertificatePemPath = _material.WriteCombinedPem("client-chain.pem", client, intermediate)
        });

        authenticationOptions.ClientCertificateContext!.IntermediateCertificates.Should().ContainSingle()
            .Which.Thumbprint.Should().Be(intermediate.Thumbprint);
    }

    [Fact]
    public void Create_preserves_intermediate_certificates_from_a_PKCS12_identity_bundle()
    {
        var intermediate = _material.IssueCertificateAuthority(_certificateAuthority, "Canton Intermediate Root");
        var client = _material.IssueClientCertificate(intermediate, "Canton Intermediate Client");
        ...
        var authenticationOptions = SslClientAuthenticationOptionsFactory.Create(new TlsOptions
        {
            ClientCertificatePkcs12Path = _material.WritePkcs12("client-chain.pfx", client, "hunter2", intermediate),
