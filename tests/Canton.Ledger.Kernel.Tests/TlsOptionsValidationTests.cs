...
public sealed class TlsOptionsValidationTests : IDisposable
{
    ...
    [Fact]
    public void RevocationMode_is_rejected_when_undefined()
    {
        var results = Validate(new TlsOptions
        {
            RevocationMode = (X509RevocationMode)99,
            CertificateAuthorities = [_material.CreateCertificateAuthority("root")]
        });
    ...
    [Fact]
    public void ClientCertificate_is_rejected_when_it_has_no_private_key()
    {
        var certificateWithPrivateKey = _material.CreateSelfSigned("client");
        using var certificateWithoutPrivateKey = X509CertificateLoader.LoadCertificate(
            certificateWithPrivateKey.Export(X509ContentType.Cert));

        var results = Validate(new TlsOptions { ClientCertificate = certificateWithoutPrivateKey });

        results.Should().ContainSingle()
            .Which.MemberNames.Should().Contain(nameof(TlsOptions.ClientCertificate));
    }
