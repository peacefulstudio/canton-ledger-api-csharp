...
public sealed class ActAsRightsLease : IAsyncDisposable
{
    ...
    /// this lease is disposed. Once the request is sent, an ambiguous failure keeps the party in the
    ...
    public async Task GrantAsync(string partyId, CancellationToken cancellationToken = default)
    {
        await SendAsync(
                ...
                [partyId],
                cancellationToken,
                beforeSend: () => _leasedParties.Add(partyId),
                responseReceived: response =>
                {
                    if (response.IsSuccessStatusCode)
                        _confirmedParties.Add(partyId);
                    else if (IsDefinitiveGrantRejection(response.StatusCode))
                        _leasedParties.Remove(partyId);
                })
    ...
    private void AssertNothingStillHeld(IReadOnlyList<string> confirmed, string response)
    {
        var stillHeld = confirmed.Except(RevokedParties(response), StringComparer.Ordinal).ToArray();
        ...
        throw new InvalidOperationException(
            ...
            + $"{confirmed.Count - stillHeld.Length} of {confirmed.Count} confirmed act-as right(s); the user "
    ...
    private static bool IsDefinitiveGrantRejection(HttpStatusCode statusCode) =>
        statusCode != HttpStatusCode.RequestTimeout && (int)statusCode is >= 400 and < 500;
