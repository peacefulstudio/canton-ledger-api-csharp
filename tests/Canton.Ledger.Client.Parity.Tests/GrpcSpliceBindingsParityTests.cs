...
public sealed class GrpcSpliceBindingsParityTests : SpliceBindingsParityTests
{
        ...
        OpenOperatorLaneAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var validatorUser = await services.GetRequiredService<IAdminClient>()
                .GetUserAsync(fixture.ValidatorUserId, cancellationToken).ConfigureAwait(false);
            Assert.False(
                string.IsNullOrWhiteSpace(validatorUser?.PrimaryParty),
                $"validator user '{fixture.ValidatorUserId}' has no primary party to read its ValidatorLicense as");

            var client = services.GetRequiredService<ICantonLedgerClient>();
            return new CapabilityLane<(ICantonLedgerClient, Party)>(
                (client, new Party(validatorUser!.PrimaryParty!)),
                async () =>
                {
                    try
                    {
