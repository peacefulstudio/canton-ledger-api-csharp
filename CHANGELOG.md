...
- **Bumped `Daml.Ledger.Abstractions` and `Daml.Runtime` to `0.1.7`.** Picks up the `ILedgerClient` breaking change that removes `ExerciseAsync` from the interface and promotes it to `LedgerClientExtensions` extension methods. `LedgerClient` now implements `TryExerciseAsync<TResult>` — the structured-outcome overload — and the throwing `ExerciseAsync` convenience wrappers continue to work unchanged for callers who have not yet migrated to outcome-based handling.
