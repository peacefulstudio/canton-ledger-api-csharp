...
- **`LedgerClient` and `AdminClient` now log a `Warning` at construction time when running in unauthenticated mode** (`ITokenProvider.None`). The message names the concrete step to take — register an `ITokenProvider` or use the `AddLedgerClient` overload that accepts `authConfiguration` — so misconfigured apps surface the oversight immediately instead of failing with an opaque gRPC `UNAUTHENTICATED` error at the first API call.
