...
- **`ClientCredentialsProvider` now resolves its token endpoint at construction** (`Canton.Ledger.Auth`, #107). Misconfigured options — neither `TokenEndpoint` nor a valid `Domain` — throw `InvalidOperationException` when the provider is constructed (for DI users: when `ITokenProvider` is first resolved) instead of at the first `GetTokenAsync` call. Hosted apps using `AddCantonAuth` see no change: `ValidateOnStart` already rejects such options at startup.
