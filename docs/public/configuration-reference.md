...
|---|---|---|---|
| `GrpcAddress` | `string` | — (required) | gRPC endpoint, e.g. `https://localhost:5001`. An `http` address opens a cleartext channel — bearer tokens are then sent readable on the wire and the client logs a warning at construction. Configured `Tls` requires `https`; validation rejects TLS material that a plaintext channel would ignore. |
...
| `Tls:ClientCertificatePemPath` | `string?` | `null` | Path to the PEM file holding the client certificate and any intermediates. Doubles as the key file when `ClientCertificateKeyPemPath` is unset or blank. One client-identity source at most: this, `ClientCertificatePkcs12Path`, or a code-only `ClientCertificate`. |
| `Tls:ClientCertificateKeyPemPath` | `string?` | `null` | Path to the client certificate's private key when it lives in a file of its own. Requires `ClientCertificatePemPath`; an empty or whitespace value selects the combined PEM instead. |
| `Tls:ClientCertificatePkcs12Path` | `string?` | `null` | Path to the PKCS#12 (`.pfx` / `.p12`) file holding the client certificate, its private key and any intermediates. |
...
| `Tls:RevocationMode` | `X509RevocationMode` | `NoCheck` | How the peer certificate's revocation status is checked: `NoCheck`, `Offline` or `Online`. Undefined numeric values are rejected. It rides the chain policy built only for custom root trust, so anything other than `NoCheck` requires an authority source and is otherwise rejected at startup. |
| `EmitPartyAndContractSpanTags` | `bool` | `false` | Opt-in: puts the party and contract identifiers of a traced call on its span — `canton.submitter.act_as`, `canton.submitter.read_as`, `canton.party_id`, `canton.party_id_hint` and `daml.contract_id`. Off by default, so a tracing backend never sees who transacted on which contract; every other span attribute is emitted either way. |
...
| `Tls:ClientCertificatePemPath` | `string?` | `null` | Path to the PEM file holding the token client's certificate and any intermediates. Doubles as the key file when `ClientCertificateKeyPemPath` is unset or blank. One client-identity source at most: this, `ClientCertificatePkcs12Path`, or a code-only `ClientCertificate`. |
| `Tls:ClientCertificateKeyPemPath` | `string?` | `null` | Path to the token client's private key when it lives in a file of its own. Requires `ClientCertificatePemPath`; an empty or whitespace value selects the combined PEM instead. |
| `Tls:ClientCertificatePkcs12Path` | `string?` | `null` | Path to the PKCS#12 (`.pfx` / `.p12`) file holding the token client's certificate, private key and any intermediates. |
| `Tls:ClientCertificatePkcs12Password` | `string?` | `null` | Password protecting `ClientCertificatePkcs12Path`; leave unset for an unprotected file. Requires `ClientCertificatePkcs12Path`. |
...
| `Tls:RevocationMode` | `X509RevocationMode` | `NoCheck` | How the identity provider certificate's revocation status is checked. Undefined numeric values are rejected, and anything other than `NoCheck` requires an authority source. |
...
| Key | Type | Default | Notes |
|---|---|---|---|
| `HttpAddress` | `string` | — (required) | JSON Ledger API base address, e.g. `http://localhost:7575`. Configured `Tls` requires `https`; validation rejects TLS material that a plaintext client would ignore. Unconfigured TLS still permits `http` for deliberate local development. |
...
| `StreamWindowIdleTimeout` | `TimeSpan` | `00:00:00.250` | How long the participant holds one window open once no further entry arrives, sent as `stream_idle_timeout_ms` and rounded down to whole milliseconds. A live tail delivers an entry only once its window closes, so this also bounds how far a followed stream trails the ledger; a quiet followed stream costs about four requests a second at the default. Must be positive, and raising it past the participant's own request timeout ends the stream instead of holding the window longer. |
